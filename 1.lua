...
-- FIX: RF/* kadang RemoteFunction, kadang RemoteEvent -> deteksi otomatis
...
-- FIX: Tambahan remote untuk Auto Equip Rod
local RE_EquipToolFromHotbar = GetServerRemote("RE/EquipToolFromHotbar")

-- FIX: Tambahan remote untuk Fishing Radar
...
-- [2] AUTO EQUIP ROD - FIX: Menggunakan RE_EquipToolFromHotbar
...
-- [3] BYPASS RADAR - FIX: Menggunakan RF_UpdateFishingRadar
local function applyBypassRadar(on)
    if not RF_UpdateFishingRadar then return end
    
    pcall(function()
        RF_UpdateFishingRadar:InvokeServer(on)
    end)
end
...
local function CreateStreePanel()
    ...
    main.Position = UDim2.new(0.5, -110, 0, 10)
...
-- [7] DISABLE OBTAINED FISH NOTIFICATION
...
local function favoriteUuid(uuid)
    ...
    elseif Events.favorite and Events.favorite:IsA("RemoteFunction") then
        pcall(function() Events.favorite:InvokeServer(uuid) end)
    end
end

-- auto favorit watcher
do
